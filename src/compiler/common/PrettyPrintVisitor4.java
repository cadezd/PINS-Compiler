...
public class PrettyPrintVisitor4 implements Visitor {
     ...
     */
    public Optional<NodeDescription<Frame>> frames = Optional.empty();

    /**
     * Razrešeni dostopi spremenljivk.
     */
    public Optional<NodeDescription<Access>> accesses = Optional.empty();
    ...
    private void printFrame(Ast funDef) {
        if (frames.isPresent()) {
            var frame = frames.get().valueFor(funDef);
            if (frame.isEmpty()) {
                throw new RuntimeException(funDef.toString());
            }
            print("# framed as: ", frame.get().toString(), "\n");
        }
    }

    private void printAccessedAs(Ast def) {
        if (accesses.isPresent()) {
            var access = accesses.get().valueFor(def);
            if (access.isEmpty()) {
                throw new RuntimeException(access.toString());
            }
            print("# accessed as: ", access.get().toString(), "\n");
        }
    }
}
